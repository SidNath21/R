...
nycflights %>%
  group_by(tailnum) %>% arrange(desc(avg_speed))
```
...
ggplot(nycflights, aes(x = factor(distance), y = avg_speed)) +
  geom_point()
```
...
nycflights <- nycflights %>% 
  mutate(arr_type = ifelse(arr_time <= dep_time + air_time, "on time", "delayed"))

nycflights <- nycflights %>% 
  mutate(dep_type = ifelse(arr_delay <= 0, "on time", "delayed"))

nycflights %>% 
  group_by(dep_type == "delayed") %>% 
  summarise(arr_percentage = sum(arr_type == "on time") / sum(dep_type == "delayed"), n = n())
```
